...
function typeNodeForIdentifier(definition, name, state, loose = false) {
    switch (definition.type) {
        case thrift_parser_1.SyntaxType.StructDefinition:
        case thrift_parser_1.SyntaxType.ExceptionDefinition:
        case thrift_parser_1.SyntaxType.UnionDefinition:
            if (loose) {
                return ts.createTypeReferenceNode(ts.createIdentifier(utils_1.looseName(name, definition.type, state)), undefined);
            }
            else {
                return ts.createTypeReferenceNode(ts.createIdentifier(utils_1.strictName(name, definition.type, state)), undefined);
            }
        default:
            return ts.createTypeReferenceNode(ts.createIdentifier(resolver_1.Resolver.resolveIdentifierName(name, {
                currentNamespace: state.currentNamespace,
...
function typeNodeForFieldType(fieldType, state, loose = false) {
    switch (fieldType.type) {
        case thrift_parser_1.SyntaxType.Identifier:
            ...
            return typeNodeForIdentifier(result.definition, fieldType.value, state, loose);
        case thrift_parser_1.SyntaxType.SetType:
        ...
        case thrift_parser_1.SyntaxType.I64Keyword:
            if (loose === true) {
                return ts.createUnionTypeNode([
                    ...
                    types_1.createStringType(),
                    ts.createTypeReferenceNode(identifiers_1.COMMON_IDENTIFIERS.Int64, undefined),
                ]);
            }
            else {
                return ts.createTypeReferenceNode(identifiers_1.COMMON_IDENTIFIERS.Int64, undefined);
            }
        case thrift_parser_1.SyntaxType.BinaryKeyword:
            if (loose === true) {
                return ts.createUnionTypeNode([
                    types_1.createStringType(),
                    types_1.createBufferType(),
                ]);
            }
            else {
